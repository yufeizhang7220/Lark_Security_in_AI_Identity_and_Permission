...
- **参数说明**：
  ...
  - `AgentSecret`: 注册时的密钥
  - `purpose`: 申请令牌的用途说明
  - `scope`: 权限范围，格式为dict(str, list(str))，数据类型支持doc/tablebase/calendar/online（与USERS\_table.json中定义的一致），支持的操作：read/write/create/delete/web\_search/fetch\_content/analyze\_content/all
...
- 令牌表：`../Storage/TOKENS_table.json` 存储已颁发的AccessToken，以`token_id`作为key，每个token\_id对应一个AccessToken记录，支持同一Agent同时存在多个有效令牌
